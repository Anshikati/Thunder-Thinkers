...
class _LoginScreenState extends State<LoginScreen> {
  ...
  Future<void> _googleSignIn() async {
    try {
      ...
      final appUser = await userService.createOrFetchGoogleUser(credential: credential);
      
      if (!mounted) return;

      if (appUser == null) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Please complete your profile with role selection')),
        );
        return;
      }

      final route = switch (appUser.role) {
  ...
  Future<void> _login() async {
    ...
    if (success) {
      context.go(switch (_selectedRole) { UserRole.ngoAdmin => '/admin', UserRole.fieldWorker => '/fieldworker', UserRole.volunteer => '/volunteer' });
                ...
                SizedBox(
                  ...
                  child: OutlinedButton.icon(
                    ...
                    icon: Padding(
                      ...
                      child: Text('G', style: TextStyle(fontWeight: FontWeight.w900, color: theme.colorScheme.onPrimaryContainer, fontSize: 16)),
                    ),
                    label: Text('Continue with Google', style: TextStyle(color: theme.colorScheme.onPrimaryContainer)),
                  ),
                ),
                const SizedBox(height: 24),
                Center(
...
class _RoleSelector extends StatelessWidget {
  ...
  static const _roles = [
    (UserRole.ngoAdmin, Icons.admin_panel_settings_outlined, 'NGO'),
